...
class LastCardGame(Game, TurnTimerMixin):
    ...
    def _action_play_selected(self, player: Player, action_id: str) -> None:
        ...
        if not p.selected_cards:
            ...
            if user:
                user.speak_l("lastcard-multi-no-cards", buffer="game")
            return
        ...
        if len(ranks) != 1:
            ...
            user = self.get_user(p)
            if user:
                user.speak_l("lastcard-multi-same-rank", buffer="game")
            return
        rank = ranks.pop()
        ...
        if rank in (RANK_WILD, RANK_WILD_DRAW_FOUR) and len(selected) > 1:
            ...
            user = self.get_user(p)
            if user:
                user.speak_l("lastcard-multi-no-wilds", buffer="game")
            return
        ...
        if playable_idx is None:
            ...
            user = self.get_user(p)
            if user:
                user.speak_l("lastcard-multi-not-playable", buffer="game")
            return
        if playable_idx > 0:
    ...
    def _broadcast_multi_play(self, player: LastCardPlayer, cards: list[Card]) -> None:
        for p in self.players:
            ...
            user.speak_l("lastcard-player-plays-multi", buffer="game",
    ...
    def _action_buzzer(self, player: Player, action_id: str) -> None:
        ...
        if self.interrupt_phase != "last_card_callout":
            ...
            if self.current_player == player and len(player.hand) == 2:
                ...
                user = self.get_user(player)
                if user:
                    user.speak_l("lastcard-you-called", buffer="game")
                return
            return
        ...
        if player.id == self.interrupt_target_id:
            ...
            if target.called_last_card:
                ...
                user = self.get_user(player)
                if user:
                    user.speak_l("lastcard-already-called", buffer="game")
                return
    ...
    def _action_jump_in(self, player: Player, action_id: str) -> None:
        ...
        if not matching:
            user = self.get_user(player)
            if user:
                user.speak_l("lastcard-no-matching-card", buffer="game")
            return
    ...
    def _action_read_hand(self, player: Player, action_id: str) -> None:
        ...
        locale = user.locale
        if not player.hand:
            user.speak_l("lastcard-hand-empty", buffer="game")
            return
    ...
    def _action_check_turn_timer(self, player: Player, action_id: str) -> None:
        ...
        if remaining <= 0:
            user.speak_l("lastcard-timer-disabled", buffer="game")
        else:
            user.speak_l("lastcard-timer-remaining", buffer="game", seconds=remaining)
    ...
    def _action_read_draw_penalty(self, player: Player, action_id: str) -> None:
        ...
        if not user:
            return
        if self.pending_draw_count > 0:
            user.speak_l("lastcard-draw-penalty-active", buffer="game",
                         count=self.pending_draw_count)
        else:
            user.speak_l("lastcard-draw-penalty-none", buffer="game")
    ...
    def _broadcast_start_card(self) -> None:
        for p in self.players:
            ...
            if not user:
                continue
            user.speak_l("lastcard-start-card", buffer="game",
    ...
    def _broadcast_color_chosen(self, color: int) -> None:
        for p in self.players:
            ...
            if not user:
                continue
            user.speak_l("lastcard-color-chosen", buffer="game",
    ...
    def _broadcast_play(self, player: LastCardPlayer, card: Card) -> None:
        for p in self.players:
            ...
            if not user:
                continue
            user.speak_l("lastcard-player-plays", buffer="game",
    ...
    def _broadcast_pass(self, player: LastCardPlayer) -> None:
        for p in self.players:
            ...
            if not user:
                continue
            user.speak_l("lastcard-player-passes", buffer="game", player=player.name)
